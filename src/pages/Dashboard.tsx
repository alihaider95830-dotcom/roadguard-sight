    ...
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Real-time tire inspection monitoring</p>
        </div>
        <Button variant="outline" onClick={fetchData} disabled={isLoading} size="sm" className="self-start sm:self-auto">
        ...
        {isLoading ? (
          ...
          <>
            ...
            <StatCard
              ...
              subtitle={`${stats ? ((stats.safeCount / (stats.totalInspections || 1)) * 100).toFixed(0) : 0}%`}
              icon={Shield}
              iconClass="text-safe"
            ...
            <StatCard
              ...
              subtitle={`${stats?.alertsPending || 0} pending`}
              icon={AlertTriangle}
      ...
      <CardContent className="pt-4 sm:pt-6 pb-4">
        <div className="flex items-start justify-between">
          <div className="space-y-0.5 sm:space-y-1 min-w-0 flex-1">
            ...
            <p className="text-xl sm:text-3xl font-bold font-mono">{value}</p>
          ...
          <div
            className={cn(
              'h-8 w-8 sm:h-12 sm:w-12 rounded-full bg-muted flex items-center justify-center shrink-0',
              pulse && 'pulse-unsafe'
            )}
          >
            <Icon className={cn('h-4 w-4 sm:h-6 sm:w-6', iconClass)} />
          </div>
        </div>
      </CardContent>
    </Card>
